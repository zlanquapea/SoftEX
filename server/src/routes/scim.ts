...
export function scimAdminRouter(ctx: Ctx) {
  const r = Router();
  r.get('/admin/scim', (req, res) => {
    const auth = authOf(req);
    requireRole(auth, 'admin');
    const ws = ctx.db.get('SELECT scim_token_hash FROM workspaces WHERE id = ?', auth.workspaceId)!;
  ...
  r.delete('/admin/scim/token', (req, res) => {
    const auth = authOf(req);
    requireRole(auth, 'admin');
    ctx.db.update('workspaces', auth.workspaceId, { scim_token_hash: null });
    audit(ctx, auth.workspaceId, auth.userId, 'scim.disabled', 'workspace', auth.workspaceId);
    res.json({ ok: true });
  });
  return r;
}
...
export function scimRouter(ctx: Ctx) {
  ...
  const loadMember = (workspaceId: string, id: string) => {
    const u = db.get(
  ...
  const setActive = (workspaceId: string, userId: string, active: boolean, role: string) => {
    ...
    if (!active) {
      ...
      db.run('UPDATE api_tokens SET revoked_at = ? WHERE user_id = ? AND workspace_id = ? AND revoked_at IS NULL', now(), userId, workspaceId);
      ctx.hub.disconnect(workspaceId, userId);
    }
    audit(ctx, workspaceId, null, active ? 'scim.user_reactivated' : 'scim.user_deactivated', 'user', userId);
  };
  ...
  r.get('/Users', (req: ScimRequest, res) => {
    ...
    const total = db.get(`SELECT COUNT(*) AS n FROM memberships m JOIN users u ON u.id = m.user_id WHERE ${where}`, ...params)!.n;
    const rows = db.all(
  ...
  r.get('/Users/:id', (req: ScimRequest, res) => {
    res.json(toScim(req.scimWorkspace!, loadMember(req.scimWorkspace!, String(req.params.id))));
  });

  r.post('/Users', (req: ScimRequest, res) => {
    const ws = req.scimWorkspace!;
    ...
    db.transaction(() => {
      ...
      for (const c of db.all(`SELECT id FROM channels WHERE workspace_id = ? AND kind IN ('public','announcement') AND name IN ('general','announcements')`, ws)) {
        db.run('INSERT OR IGNORE INTO channel_members (channel_id, user_id, joined_at) VALUES (?, ?, ?)', c.id, user!.id, now());
      }
    });
    audit(ctx, ws, null, 'scim.user_created', 'user', user!.id, { email });
    res.status(201).location(`${ctx.config.publicUrl}/scim/v2/Users/${user!.id}`).json(toScim(ws, loadMember(ws, user!.id)));
  });

  r.put('/Users/:id', (req: ScimRequest, res) => {
    const ws = req.scimWorkspace!;
    const u = loadMember(ws, String(req.params.id));
    const name = nameFrom(req.body ?? {});
    if (name) db.update('users', u.id, { name: name.slice(0, 80) });
    if (typeof req.body.title === 'string') db.update('users', u.id, { title: req.body.title.slice(0, 80) });
    if (req.body.externalId !== undefined) db.run('UPDATE memberships SET scim_external_id = ? WHERE workspace_id = ? AND user_id = ?', externalIdOf(req.body.externalId), ws, u.id);
    if (typeof req.body.active === 'boolean' && req.body.active === !!u.deactivated_at) setActive(ws, u.id, req.body.active, u.role);
    res.json(toScim(ws, loadMember(ws, u.id)));
  });

  r.patch('/Users/:id', (req: ScimRequest, res) => {
    const ws = req.scimWorkspace!;
    const u = loadMember(ws, String(req.params.id));
    ...
    for (const op of req.body.Operations ?? []) {
      ...
      for (const [path, value] of updates) {
        if (/^active$/i.test(path)) {
          ...
          if (active === !!loadMember(ws, u.id).deactivated_at) setActive(ws, u.id, active, u.role);
        } else if (/^(displayName|name\.formatted)$/i.test(path) && typeof value === 'string') {
          db.update('users', u.id, { name: value.slice(0, 80) });
        } else if (/^title$/i.test(path) && typeof value === 'string') {
          db.update('users', u.id, { title: value.slice(0, 80) });
        } else if (/^externalId$/i.test(path)) {
          db.run('UPDATE memberships SET scim_external_id = ? WHERE workspace_id = ? AND user_id = ?', externalIdOf(value), ws, u.id);
        }
      }
    }
    res.json(toScim(ws, loadMember(ws, u.id)));
  ...
  r.delete('/Users/:id', (req: ScimRequest, res) => {
    const ws = req.scimWorkspace!;
    const u = loadMember(ws, String(req.params.id));
    // Deprovisioning deactivates rather than erases, so content and audit history stay intact.
    if (!u.deactivated_at) setActive(ws, u.id, false, u.role);
    res.status(204).end();
  });

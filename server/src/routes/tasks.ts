...
export async function serializeTasks(db: Database, rows: Row[]) {
  ...
  const comments = await db.all(`SELECT task_id, COUNT(*) AS n FROM task_comments WHERE task_id IN (${marks}) GROUP BY task_id`, ...ids);
  const subtasks = await db.all(
    `SELECT parent_id, COUNT(*) AS total, SUM(CASE WHEN status = 'done' THEN 1 ELSE 0 END) AS done FROM tasks WHERE parent_id IN (${marks}) GROUP BY parent_id`,
    ...ids,
  );
  const openDeps = await db.all(
  ...
  const projects = new Map(
    projectIds.length
      ? (await db.all(`SELECT id, name, color FROM projects WHERE id IN (${projectIds.map(() => '?').join(',')})`, ...projectIds)).map((p) => [p.id, p])
      : [],
  );
...
export function tasksRouter(ctx: Ctx) {
  ...
  const checkAssignee = async (auth: Auth, userId: string | null | undefined, project: Row | null) => {
    if (!userId) return;
    ...
    if (project && (project.visibility === 'private' || m.role === 'guest') && !await isProjectMember(db, project.id, userId)) {
  ...
  const createTask = async (auth: Auth, body: z.infer<typeof TaskInput>) => {
    ...
    if (body.milestoneId && !await db.get('SELECT 1 FROM milestones WHERE id = ? AND project_id = ?', body.milestoneId, body.projectId ?? '')) {
      throw badRequest('Milestone does not belong to this project');
    }
    const ownerId = body.ownerId === undefined ? auth.userId : body.ownerId;
    await checkAssignee(auth, ownerId, project);
    await checkAssignee(auth, body.reviewerId, project);
    if (body.sourceMessageId) {
      const message = await db.get('SELECT * FROM messages WHERE id = ?', body.sourceMessageId);
    ...
    const id = newId();
    const position = ((await db.get('SELECT MAX(position) AS p FROM tasks WHERE project_id IS ? AND status = ?', body.projectId ?? null, body.status))?.p ?? 0) + 1;
    await db.transaction(async () => {
      ...
      for (const c of body.collaboratorIds) {
        if (await isActiveMember(db, auth.workspaceId, c)) await db.run('INSERT OR IGNORE INTO task_collaborators (task_id, user_id) VALUES (?, ?)', id, c);
      }
    ...
    if (body.sourceMessageId) {
      const message = (await db.get('SELECT * FROM messages WHERE id = ?', body.sourceMessageId))!;
      await ctx.hub.publish(auth.workspaceId, { type: 'message.updated', messageId: message.id, channelId: message.channel_id, parentId: message.parent_id });
    }
    await runAutomations(ctx, 'task.created', (await db.get('SELECT * FROM tasks WHERE id = ?', id))!);
    await publishTask(auth, id);
    return id;
  };
  ...
  r.get('/tasks', async (req, res) => {
    ...
    const rows = (await filterAsync((await db
      .all(`SELECT t.* FROM tasks t WHERE ${where.join(' AND ')} ORDER BY t.position, t.created_at LIMIT 2000`, ...params)), (t) => canViewTask(db, auth, t)));
    res.json(await serializeTasks(db, rows));
  });
  ...
  r.get('/my-work', async (req, res) => {
    ...
    const mine = (await filterAsync((await db
      .all(
    ...
    const tasks = await serializeTasks(db, mine);
  ...
  r.post('/messages/:id/task', async (req, res) => {
    const auth = authOf(req);
    ...
    const id = await createTask(auth, input);
    res.status(201).json((await serializeTasks(db, [(await db.get('SELECT * FROM tasks WHERE id = ?', id))!]))[0]);
  });
  ...
  r.get('/tasks/:id', async (req, res) => {
    ...
    const [base] = await serializeTasks(db, [task]);
    ...
    const subtasks = await serializeTasks(db, await db.all('SELECT * FROM tasks WHERE parent_id = ? ORDER BY position, created_at', task.id));
    const dependsOn = (await filterAsync((await db
      .all(`SELECT t.* FROM task_dependencies d JOIN tasks t ON t.id = d.depends_on_id WHERE d.task_id = ?`, task.id)), (t) => canViewTask(db, auth, t)))
      .map((t) => ({ id: t.id, title: t.title, status: t.status }));
    const blocking = (await filterAsync((await db
      .all(`SELECT t.* FROM task_dependencies d JOIN tasks t ON t.id = d.task_id WHERE d.depends_on_id = ?`, task.id)), (t) => canViewTask(db, auth, t)))
      .map((t) => ({ id: t.id, title: t.title, status: t.status }));
    ...
    if (task.source_message_id) {
      const m = await db.get('SELECT m.id, m.channel_id, m.body, c.name AS channel_name, c.kind FROM messages m JOIN channels c ON c.id = m.channel_id WHERE m.id = ?', task.source_message_id);
      if (m) {
        try {
          await loadChannel(db, auth, m.channel_id);
          source = m;
        } catch {
          source = null;
        }
    ...
    const meeting = task.meeting_id ? await db.get('SELECT * FROM meetings WHERE id = ?', task.meeting_id) : null;
    const milestones = task.project_id ? await db.all('SELECT id, name FROM milestones WHERE project_id = ? ORDER BY due_date', task.project_id) : [];
    ...
    res.json({
      ...
      meeting: meeting && await canViewMeeting(db, auth, meeting) ? { id: meeting.id, title: meeting.title, starts_at: meeting.starts_at } : null,
      milestones,
      files,
      can_edit: await canEditTask(db, auth, task),
      creator: await userSummary(db, task.created_by),
    });
  });
  ...
  r.patch('/tasks/:id', async (req, res) => {
    ...
    const project = task.project_id ? (await db.get('SELECT * FROM projects WHERE id = ?', task.project_id))! : null;
    if (body.ownerId !== undefined) await checkAssignee(auth, body.ownerId, project);
    if (body.reviewerId !== undefined) await checkAssignee(auth, body.reviewerId, project);
    ...
    const fresh = (await db.get('SELECT * FROM tasks WHERE id = ?', task.id))!;
    ...
    if (statusChanged) await emitEvent(ctx, auth.workspaceId, 'task.status_changed', eventData, { projectId: task.project_id });
    if (body.ownerId && body.ownerId !== task.owner_id) await emitEvent(ctx, auth.workspaceId, 'task.assigned', eventData, { projectId: task.project_id });
    if (statusChanged) {
      ...
      const watchers = new Set<string>([task.owner_id, task.created_by, ...(await db.all('SELECT user_id FROM task_collaborators WHERE task_id = ?', task.id)).map((c) => c.user_id)].filter(Boolean));
      for (const userId of watchers) {
        await notify(ctx, auth.workspaceId, { userId, kind: 'status', title: `${actor.name} moved “${task.title}” to ${label}`, link, actorId: auth.userId });
      }
      const reviewer = body.reviewerId ?? task.reviewer_id;
      if (body.status === 'review' && reviewer) {
        await notify(ctx, auth.workspaceId, { userId: reviewer, kind: 'review', title: `“${task.title}” is ready for your review`, link, actorId: auth.userId });
      }
      if (body.status === 'done') {
        ...
        for (const dep of dependents) {
          if (dep.owner_id) await notify(ctx, auth.workspaceId, { userId: dep.owner_id, kind: 'handoff', title: `“${task.title}” is done — “${dep.title}” can move forward`, link: `/tasks/${dep.id}`, actorId: auth.userId });
        }
        if (task.recurrence) {
          const nextId = newId();
          await db.insert('tasks', {
    ...
    if (body.ownerId && body.ownerId !== task.owner_id) {
      await recordActivity(ctx, auth.workspaceId, {
        ...
        objectId: task.id,
        projectId: task.project_id,
        summary: `assigned “${task.title}” to ${(await db.get('SELECT name FROM users WHERE id = ?', body.ownerId))!.name}`,
        link,
      });
      await notify(ctx, auth.workspaceId, { userId: body.ownerId, kind: 'assigned', title: `${actor.name} assigned you “${task.title}”`, link, actorId: auth.userId });
    }
    if (body.dueDate !== undefined && body.dueDate !== task.due_date && task.owner_id) {
      await notify(ctx, auth.workspaceId, { userId: task.owner_id, kind: 'status', title: `Due date for “${task.title}” changed to ${body.dueDate ?? 'none'}`, link, actorId: auth.userId });
    }
    if (statusChanged) await runAutomations(ctx, 'task.status_changed', (await db.get('SELECT * FROM tasks WHERE id = ?', task.id))!, task.status);
    await publishTask(auth, task.id);
    res.json((await serializeTasks(db, [(await db.get('SELECT * FROM tasks WHERE id = ?', task.id))!]))[0]);
  });
  ...
  r.delete('/tasks/:id', async (req, res) => {
    ...
    await ctx.hub.publish(auth.workspaceId, { type: 'task.updated', taskId: task.id, projectId: task.project_id, deleted: true });
    res.json({ ok: true });
  });

  // ----- Checklist, comments, collaborators, dependencies -----

  const editable = async (auth: Auth, id: string) => {
    const task = await loadTask(db, auth, id);
    if (!await canEditTask(db, auth, task)) throw forbidden('You cannot edit this task');
    return task;
  };

  r.post('/tasks/:id/checklist', async (req, res) => {
    const task = await editable(authOf(req), req.params.id);
    const { text } = parse(z.object({ text: z.string().trim().min(1).max(300) }), req.body);
    const id = newId();
    const pos = ((await db.get('SELECT MAX(position) AS p FROM checklist_items WHERE task_id = ?', task.id))?.p ?? 0) + 1;
    await db.insert('checklist_items', { id, task_id: task.id, text, position: pos });
    res.status(201).json(await db.get('SELECT * FROM checklist_items WHERE id = ?', id));
  });

  r.patch('/checklist/:id', async (req, res) => {
    const item = await db.get('SELECT * FROM checklist_items WHERE id = ?', req.params.id);
    if (!item) throw notFound('Checklist item');
    await editable(authOf(req), item.task_id);
    const body = parse(z.object({ text: z.string().trim().min(1).max(300).optional(), done: z.boolean().optional() }), req.body);
    await db.update('checklist_items', item.id, { text: body.text, done: body.done });
    res.json(await db.get('SELECT * FROM checklist_items WHERE id = ?', item.id));
  });

  r.delete('/checklist/:id', async (req, res) => {
    const item = await db.get('SELECT * FROM checklist_items WHERE id = ?', req.params.id);
    if (!item) throw notFound('Checklist item');
    await editable(authOf(req), item.task_id);
  ...
  r.post('/tasks/:id/comments', async (req, res) => {
    ...
    const id = newId();
    await db.insert('task_comments', { id, task_id: task.id, user_id: auth.userId, body, created_at: now() });
    const actor = (await db.get('SELECT name FROM users WHERE id = ?', auth.userId))!;
    const people = new Set<string>([
      task.owner_id,
      task.created_by,
      task.reviewer_id,
      ...(await db.all('SELECT user_id FROM task_collaborators WHERE task_id = ?', task.id)).map((c) => c.user_id),
      ...(await db.all('SELECT DISTINCT user_id FROM task_comments WHERE task_id = ?', task.id)).map((c) => c.user_id),
    ].filter(Boolean));
    for (const userId of people) {
      await notify(ctx, auth.workspaceId, { userId, kind: 'comment', title: `${actor.name} commented on “${task.title}”`, body, link: `/tasks/${task.id}`, actorId: auth.userId });
    }
    await publishTask(auth, task.id);
    res.status(201).json(await db.get('SELECT * FROM task_comments WHERE id = ?', id));
  });

  r.put('/tasks/:id/collaborators', async (req, res) => {
    const auth = authOf(req);
    const task = await editable(auth, req.params.id);
    const { userIds } = parse(z.object({ userIds: z.array(z.string()).max(50) }), req.body);
    const project = task.project_id ? (await db.get('SELECT * FROM projects WHERE id = ?', task.project_id))! : null;
    for (const u of userIds) await checkAssignee(auth, u, project);
    await db.transaction(async () => {
      await db.run('DELETE FROM task_collaborators WHERE task_id = ?', task.id);
      for (const u of userIds) await db.run('INSERT OR IGNORE INTO task_collaborators (task_id, user_id) VALUES (?, ?)', task.id, u);
    });
    res.json({ ok: true });
  });

  r.post('/tasks/:id/dependencies', async (req, res) => {
    const auth = authOf(req);
    const task = await editable(auth, req.params.id);
    ...
    await loadTask(db, auth, dependsOnId);
    ...
    while (stack.length) {
      ...
      seen.add(current);
      stack.push(...(await db.all('SELECT depends_on_id FROM task_dependencies WHERE task_id = ?', current)).map((d) => d.depends_on_id));
    }
    await db.run('INSERT OR IGNORE INTO task_dependencies (task_id, depends_on_id) VALUES (?, ?)', task.id, dependsOnId);
    res.status(201).json({ ok: true });
  });

  r.delete('/tasks/:id/dependencies/:dependsOnId', async (req, res) => {
    const task = await editable(authOf(req), req.params.id);
    await db.run('DELETE FROM task_dependencies WHERE task_id = ? AND depends_on_id = ?', task.id, req.params.dependsOnId);
    res.json({ ok: true });
  });

  /** Tasks visible to the user for dependency pickers. */
  r.get('/tasks-lookup', async (req, res) => {
    const auth = authOf(req);
    ...
    const rows = (await filterAsync((await db
      .all(
        ...
        auth.workspaceId,
        `%${q.q}%`,
      )), async (t) => (t.project_id ? projects.includes(t.project_id) : await canViewTask(db, auth, t))))
      .slice(0, 20);

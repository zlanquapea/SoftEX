...
import { forbidden, likePattern, newId, now, parse, parseJson, today } from '../util.js';
...
export function homeRouter(ctx: Ctx) {
  ...
  r.get('/home', (req, res) => {
    ...
    const t0 = today();
    const membership = db.get('SELECT last_seen_home_at FROM memberships WHERE workspace_id = ? AND user_id = ?', auth.workspaceId, auth.userId)!;
    ...
    const channelSet = new Set(accessibleChannelIds(db, auth));
    ...
    const memberProjects = db.all('SELECT project_id FROM project_members WHERE user_id = ?', auth.userId).map((p) => p.project_id).filter((p) => projectSet.has(p));
    const blocked = db
      .all(
    ...
    const meetings = db
      .all(
        `SELECT m.*, p.response FROM meetings m JOIN meeting_participants p ON p.meeting_id = m.id AND p.user_id = ?
          WHERE m.workspace_id = ? AND m.ended_at IS NULL AND datetime(m.starts_at, '+' || m.duration_min || ' minutes') >= datetime(?)
      ...
      .map((m) => ({
        ...
        response: m.response,
        participants: db.all(`SELECT u.id, u.name, u.color FROM meeting_participants mp JOIN users u ON u.id = mp.user_id WHERE mp.meeting_id = ? LIMIT 5`, m.id),
      }));
    ...
    const activeProjects = memberProjects.length
      ? db
          ...
          .map((p) => {
            const counts = db.get(
              `SELECT COUNT(*) AS total, SUM(status = 'done') AS done FROM tasks WHERE project_id = ? AND parent_id IS NULL`,
              p.id,
            )!;
            return {
              id: p.id,
              ...
              members: db.all(`SELECT u.id, u.name, u.color FROM project_members m JOIN users u ON u.id = m.user_id WHERE m.project_id = ? LIMIT 4`, p.id),
              member_count: db.get('SELECT COUNT(*) AS n FROM project_members WHERE project_id = ?', p.id)!.n,
            };
          })
      : [];

    const onboarding = db.get(
      `SELECT COUNT(*) AS total, SUM(p.done_at IS NOT NULL) AS done FROM onboarding_items i
    ...
    const pendingApprovals = db.get(`SELECT COUNT(*) AS n FROM requests WHERE approver_id = ? AND workspace_id = ? AND status = 'pending'`, auth.userId, auth.workspaceId)!.n;
    const checkedInToday = !!db.get('SELECT 1 FROM checkins WHERE user_id = ? AND workspace_id = ? AND created_at >= ?', auth.userId, auth.workspaceId, `${t0}T00:00:00`);
    const reviewsDue = db
    ...
    res.json({
      since,
      focus: serializeTasks(db, focus),
      up_next: serializeTasks(db, upNextTasks),
      blocked: serializeTasks(db, blocked),
      meetings,
      mentions,
      decisions,
      changes,
      ...
      counts: {
        ...
        unread: db.get('SELECT COUNT(*) AS n FROM notifications WHERE user_id = ? AND workspace_id = ? AND read_at IS NULL', auth.userId, auth.workspaceId)!.n,
      },
    });
  });

  r.post('/home/seen', (req, res) => {
    const auth = authOf(req);
    db.run('UPDATE memberships SET last_seen_home_at = ? WHERE workspace_id = ? AND user_id = ?', now(), auth.workspaceId, auth.userId);
    res.json({ ok: true });
  });
  ...
  r.get('/notifications', (req, res) => {
    ...
    const unread = db.get('SELECT COUNT(*) AS n FROM notifications WHERE user_id = ? AND workspace_id = ? AND read_at IS NULL', auth.userId, auth.workspaceId)!.n;
  ...
  r.post('/notifications/:id/read', (req, res) => {
    ...
    db.run('UPDATE notifications SET read_at = ? WHERE id = ? AND user_id = ?', read ? now() : null, req.params.id, auth.userId);
    res.json({ ok: true });
  });

  r.post('/notifications/read-all', (req, res) => {
    const auth = authOf(req);
    db.run('UPDATE notifications SET read_at = ? WHERE user_id = ? AND workspace_id = ? AND read_at IS NULL', now(), auth.userId, auth.workspaceId);
    res.json({ ok: true });
  });

  /** Digest of the last day, grouped by kind, to reduce interruption load (§5.5). */
  r.get('/notifications/digest', (req, res) => {
    const auth = authOf(req);
    const since = new Date(Date.now() - 86_400_000).toISOString();
    res.json(
      db.all(
        `SELECT kind, COUNT(*) AS count, SUM(read_at IS NULL) AS unread, MAX(created_at) AS latest FROM notifications
  ...
  r.get('/activity', (req, res) => {
    ...
    const projectSet = new Set(accessibleProjectIds(db, auth));
    const channelSet = new Set(accessibleChannelIds(db, auth));
    const rows = db
      .all(
        `SELECT a.*, u.name AS actor_name, u.color AS actor_color FROM activity a JOIN users u ON u.id = a.actor_id
  ...
  r.post('/checkins', (req, res) => {
    ...
    const id = newId();
    db.insert('checkins', { id, workspace_id: auth.workspaceId, project_id: body.projectId ?? null, user_id: auth.userId, done: body.done, next: body.next, blockers: body.blockers, created_at: now() });
    if (project) {
      recordActivity(ctx, auth.workspaceId, { actorId: auth.userId, verb: 'checked in', objectType: 'checkin', objectId: id, projectId: project.id, summary: `checked in on ${project.name}`, link: `/projects/${project.id}?tab=checkins` });
      if (body.blockers.trim()) {
        const name = db.get('SELECT name FROM users WHERE id = ?', auth.userId)!.name;
        notify(ctx, auth.workspaceId, { userId: project.owner_id, kind: 'status', title: `${name} reported a blocker on ${project.name}`, body: body.blockers, link: `/projects/${project.id}?tab=checkins`, actorId: auth.userId });
      }
    }
    res.status(201).json(db.get('SELECT * FROM checkins WHERE id = ?', id));
  });
  ...
  r.get('/search', (req, res) => {
    ...
    const channels = accessibleChannelIds(db, auth).filter((c) => !q.channelId || c === q.channelId);
    const projects = accessibleProjectIds(db, auth).filter((p) => !q.projectId || p === q.projectId);

...
async function creatorAuth(ctx: Ctx, rule: Row): Promise<Auth | null> {
  const m = await ctx.db.get(
...
export async function runAutomations(ctx: Ctx, trigger: TriggerType, task: Row, previousStatus?: string) {
  if (!task.project_id || !await hasFeature(ctx, task.workspace_id, 'automations')) return;
  const { db } = ctx;
  const rules = await db.all('SELECT * FROM automations WHERE project_id = ? AND trigger_type = ? AND enabled = 1', task.project_id, trigger);
  for (const rule of rules) {
    ...
    let detail = '';
    try {
      detail = await applyAction(ctx, rule, (await db.get('SELECT * FROM tasks WHERE id = ?', task.id))!);
    } catch (error) {
      outcome = error instanceof SkipError ? 'skipped' : 'error';
      detail = (error as Error).message;
    }
    await db.insert('automation_runs', { id: newId(), automation_id: rule.id, task_id: task.id, outcome, detail: detail.slice(0, 300), created_at: now() });
    await db.run('UPDATE automations SET run_count = run_count + 1, last_run_at = ? WHERE id = ?', now(), rule.id);
    await db.run(
...
async function applyAction(ctx: Ctx, rule: Row, task: Row): Promise<string> {
  const { db } = ctx;
  const auth = await creatorAuth(ctx, rule);
  const project = (await db.get('SELECT * FROM projects WHERE id = ?', task.project_id))!;
  if (!auth || !await canViewProject(db, auth, project)) throw new SkipError('The rule’s creator no longer has access to this project');
  ...
  switch (rule.action_type) {
    case 'assign': {
      ...
      if (!userId || !await isActiveMember(db, rule.workspace_id, userId)) throw new SkipError('No active person to assign');
      if (task.owner_id === userId) throw new SkipError('Already assigned');
      const person = (await db.get('SELECT m.role FROM memberships m WHERE m.workspace_id = ? AND m.user_id = ?', rule.workspace_id, userId))!;
      if (!await canViewProject(db, { userId, workspaceId: rule.workspace_id, role: person.role }, project)) throw new SkipError('That person cannot see this project');
      await db.update('tasks', task.id, { owner_id: userId, updated_at: now() });
      await notify(ctx, rule.workspace_id, { userId, kind: 'assigned', title: `${label} assigned you “${task.title}”`, link: `/tasks/${task.id}` });
      return `Assigned to ${(await db.get('SELECT name FROM users WHERE id = ?', userId))!.name}`;
    }
    case 'set_priority': {
      if (!cfg.priority || cfg.priority === task.priority) throw new SkipError('Priority unchanged');
      await db.update('tasks', task.id, { priority: cfg.priority, updated_at: now() });
      return `Priority set to ${cfg.priority}`;
    }
    case 'notify': {
      const userId = resolveUser();
      if (!userId) throw new SkipError('Nobody to notify');
      await notify(ctx, rule.workspace_id, {
        userId,
        kind: 'automation',
    ...
    case 'post_message': {
      const channel = cfg.channel_id ? await db.get('SELECT * FROM channels WHERE id = ?', cfg.channel_id) : undefined;
      if (!channel || channel.archived_at || !await canViewChannel(db, auth, channel)) throw new SkipError('Channel unavailable');
      ...
      await postMessage(ctx, auth, channel, { body: `${label}: ${text}` });
      return `Posted in #${channel.name}`;
    }
    case 'add_checklist': {
      const items = cfg.items ?? [];
      if (!items.length) throw new SkipError('No checklist items configured');
      const start = (await db.get('SELECT COALESCE(MAX(position), 0) AS p FROM checklist_items WHERE task_id = ?', task.id))!.p;
      await Promise.all(items.map(async (text, i) => await db.insert('checklist_items', { id: newId(), task_id: task.id, text, position: start + i + 1 })));

...
export function isActiveMember(db: Database, workspaceId: string, userId: string) {
  return !!db.get(
...
export function isChannelMember(db: Database, channelId: string, userId: string) {
  return !!db.get('SELECT 1 FROM channel_members WHERE channel_id = ? AND user_id = ?', channelId, userId);
}
...
export function canPostChannel(db: Database, auth: Auth, channel: Row) {
  if (!canViewChannel(db, auth, channel) || channel.archived_at) return false;
  if (channel.kind === 'announcement') return atLeast(auth, 'lead') || channel.created_by === auth.userId;
  return true;
}

export function loadChannel(db: Database, auth: Auth, channelId: string) {
  const channel = db.get('SELECT * FROM channels WHERE id = ?', channelId);
  if (!channel || !canViewChannel(db, auth, channel)) throw notFound('Channel');
  return channel;
}

export function accessibleChannelIds(db: Database, auth: Auth): string[] {
...
export function isProjectMember(db: Database, projectId: string, userId: string) {
  return !!db.get('SELECT 1 FROM project_members WHERE project_id = ? AND user_id = ?', projectId, userId);
}
...
export function canContributeProject(db: Database, auth: Auth, project: Row) {
  if (!canViewProject(db, auth, project) || project.archived_at) return false;
  return !isGuest(auth) || isProjectMember(db, project.id, auth.userId);
}

/** Manage a project: settings, membership, archive. */
export function canManageProject(db: Database, auth: Auth, project: Row) {
  if (!canViewProject(db, auth, project)) return false;
  if (project.owner_id === auth.userId || isAdmin(auth)) return true;
  return atLeast(auth, 'lead') && isProjectMember(db, project.id, auth.userId);
}

export function loadProject(db: Database, auth: Auth, projectId: string) {
  const project = db.get('SELECT * FROM projects WHERE id = ?', projectId);
  if (!project || !canViewProject(db, auth, project)) throw notFound('Project');
  return project;
}

export function accessibleProjectIds(db: Database, auth: Auth): string[] {
  const rows = isGuest(auth)
    ? db.all(
...
export function canViewTask(db: Database, auth: Auth, task: Row) {
  if (task.workspace_id !== auth.workspaceId) return false;
  if (task.project_id) {
    const project = db.get('SELECT * FROM projects WHERE id = ?', task.project_id);
    return !!project && canViewProject(db, auth, project);
  }
  return (
    task.owner_id === auth.userId ||
    task.created_by === auth.userId ||
    task.reviewer_id === auth.userId ||
    !!db.get('SELECT 1 FROM task_collaborators WHERE task_id = ? AND user_id = ?', task.id, auth.userId)
  );
}

export function canEditTask(db: Database, auth: Auth, task: Row) {
  if (!canViewTask(db, auth, task)) return false;
  if (!task.project_id) return true;
  const project = db.get('SELECT * FROM projects WHERE id = ?', task.project_id)!;
  return canContributeProject(db, auth, project) || task.owner_id === auth.userId;
}

export function loadTask(db: Database, auth: Auth, taskId: string) {
  const task = db.get('SELECT * FROM tasks WHERE id = ?', taskId);
  if (!task || !canViewTask(db, auth, task)) throw notFound('Task');
  return task;
}

// ---------- Knowledge, files, meetings, decisions ----------

export function canViewPage(db: Database, auth: Auth, page: Row) {
  if (page.workspace_id !== auth.workspaceId) return false;
  if (page.project_id) {
    const project = db.get('SELECT * FROM projects WHERE id = ?', page.project_id);
    return !!project && canViewProject(db, auth, project);
  }
  return !isGuest(auth);
}

export function canViewFile(db: Database, auth: Auth, file: Row) {
  if (file.workspace_id !== auth.workspaceId) return false;
  if (file.project_id) {
    const project = db.get('SELECT * FROM projects WHERE id = ?', file.project_id);
    return !!project && canViewProject(db, auth, project);
  }
  if (file.channel_id) {
    const channel = db.get('SELECT * FROM channels WHERE id = ?', file.channel_id);
    return !!channel && canViewChannel(db, auth, channel);
  }
  if (file.task_id) {
    const task = db.get('SELECT * FROM tasks WHERE id = ?', file.task_id);
    return !!task && canViewTask(db, auth, task);
  }
  return !isGuest(auth) || file.owner_id === auth.userId;
}

export function canViewMeeting(db: Database, auth: Auth, meeting: Row) {
  if (meeting.workspace_id !== auth.workspaceId) return false;
  if (meeting.organizer_id === auth.userId) return true;
  if (db.get('SELECT 1 FROM meeting_participants WHERE meeting_id = ? AND user_id = ?', meeting.id, auth.userId)) {
    return true;
  }
  if (meeting.project_id) {
    const project = db.get('SELECT * FROM projects WHERE id = ?', meeting.project_id);
    if (project && canViewProject(db, auth, project)) return true;
  }
  if (meeting.channel_id) {
    const channel = db.get('SELECT * FROM channels WHERE id = ?', meeting.channel_id);
    if (channel && canViewChannel(db, auth, channel)) return true;
  }
  return false;
}

export function canViewDecision(db: Database, auth: Auth, decision: Row) {
  if (decision.workspace_id !== auth.workspaceId) return false;
  if (decision.project_id) {
    const project = db.get('SELECT * FROM projects WHERE id = ?', decision.project_id);
    return !!project && canViewProject(db, auth, project);
  }
  if (decision.channel_id) {
    const channel = db.get('SELECT * FROM channels WHERE id = ?', decision.channel_id);
    return !!channel && canViewChannel(db, auth, channel);
  }
  if (decision.meeting_id) {
    const meeting = db.get('SELECT * FROM meetings WHERE id = ?', decision.meeting_id);
    return !!meeting && canViewMeeting(db, auth, meeting);
  }
  return !isGuest(auth);
}
